...
/** @internal */
export function generateDefaultConsts (dest = 'packages/api/src/augment/consts.ts', data = staticData, extraTypes: Record<string, Record<string, { types: Record<string, any> }>> = {}, isStrict = false, isFromChain = false): void {
  if (!isFromChain) {
    console.warn("ctt generateDefaultConsts not from chain, ignore")
    return;
  }
